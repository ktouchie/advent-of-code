...
def mark_path(row, col, direction, grid, path=None):
    ...
    if path is None:
        ...
        if (row, col, direction) in path:
            logger.error(
                f"Infinite loop detected at row {row}, col {col}, direction {direction}"
            )
    ...
    while True:
        ...
        if (next_row, next_col, direction) in path:
            logger.info(
                f"Infinite loop detected at row {row}, col {col}, direction {direction}"
            )
            raise Exception("Infinite loop detected")
        path.add((next_row, next_col, direction))
        row, col = next_row, next_col


def simulate_obstacle_placement(
    grid, path, starting_row, starting_col, starting_direction
):
    obstacles_causing_infinite_loop = set()
    for row, col, _direction in path:
        grid[row][col] = "#"
        try:
            mark_path(starting_row, starting_col, starting_direction, grid)
        except Exception:
            logger.info(f"Obstacle placed at row {row}, col {col} causes infinite loop")
            if not row == starting_row or not col == starting_col:
                obstacles_causing_infinite_loop.add((row, col))
        finally:
            grid[row][col] = "."
    logger.info(
        f"Obstacles causing infinite loop: {len(obstacles_causing_infinite_loop)}"
    )
    return obstacles_causing_infinite_loop
...
def main(input_file):
    ...
    simulate_obstacle_placement(grid, path, row, col, direction)
